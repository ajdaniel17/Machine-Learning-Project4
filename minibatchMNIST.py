...
def Gradient_Descent(DataX, DataT):
    ...
    for i in range(maxEpochs):
        ...
        for j in range(SIZE):
            temp1 = 0
            for k in range(K):
                print(np.transpose(W)[k].shape)
                print(DataX[j:batchSize][:].shape)
                temp1 += np.exp(np.matmul(np.transpose(W)[k],DataX[j:batchSize][:])- np.max(np.matmul(np.transpose(W)[k],DataX[j:batchSize][:])))

            for k in range(K):
                temp2 = (np.exp(np.matmul(np.transpose(W)[k],DataX[j:batchSize][:]) - np.max(np.matmul(np.transpose(W)[k],DataX[j:batchSize][:])))) / temp1
                temp3 = (temp2 - DataT[j:batchSize][k]) * DataX[j:batchSize][:]
