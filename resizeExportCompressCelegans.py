...
# SIZE = 28*28 #Size of the images 

# #Load all images in Data set 0 into a list
# filenames = glob.glob('C. elegans/Data/0/*.png')
# filenames.sort()
...
# resized = [cv2.resize(img, dim, interpolation=cv2.INTER_AREA) for img in images]
# grayscale = [cv2.cvtColor(img, cv2.COLOR_BGR2GRAY) for img in resized]

# path = 'C. elegans/Data/0Resized'
# i = 0
...
imgs = []
path = os.getcwd() + "/C. elegans/Data/0Resized"
valid_images = [".png"]
...
imgs = []
path = os.getcwd() + "/C. elegans/Data/1Resized"
for f in os.listdir(path):
    ext = os.path.splitext(f)[1]
    if ext.lower() not in valid_images:
        continue
...
np.savez_compressed('DataXResized.npz', DataX = DataX)
np.savez_compressed('DataTResized.npz', DataT = DataT)
